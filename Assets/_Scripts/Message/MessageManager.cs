...
public class MessageManager : MonoBehaviour
{
    ...
    {
        int totalDays = _dataService.SaveData.day + (_dataService.SaveData.month == 12 ? 30 : 0);
    ...
    {
        if (_dataService.SaveData.motherIsAlive && _dataService.SaveData.daysWithoutMeds == 1)
        {
            ScheduleMessage("MotherNeedsMedicine", "Мать", 12f);
        }

        if (_dataService.SaveData.wifeIsAlive &&
            _dataService.SaveData.childIsAlive &&
            _dataService.SaveData.child2IsAlive &&
            (_dataService.SaveData.daysWithoutFood == 1 || _dataService.SaveData.daysWithoutFood == 2))
        {
        ...
        if (_dataService.SaveData.wifeIsAlive &&
            _dataService.SaveData.childIsAlive &&
            _dataService.SaveData.child2IsAlive &&
            _dataService.SaveData.daysWithoutHeat == 1 && _dataService.SaveData.daysWithoutHeat == 2)
        {
            ScheduleMessage("WifeNeedsHeat", "Жена", Random.Range(24, 108));
        }

        if (_dataService.SaveData.daysUnpaidDebtFriends > 2)
        {
            ScheduleMessage("DebtFriends", "Друг", Random.Range(24, 108));
        }

        if (_dataService.SaveData.daysUnpaidBank == 4)
        {
        ...
        if (_dataService.SaveData.daysUnpaidMicroloan > 2)
        {
            ScheduleMessage("MicroloanPenalty", "Микрозайм", Random.Range(108, 144));
        }

        if (_dataService.SaveData.daysUnpaidCarDebt > 4)
        {
